...
def compute_portvals(df_orders, start_val = 1000000, commission=9.95, impact=0.005):
    ...
    df_prices.fillna(method='ffill').fillna(method='bfill')
    df_prices = df_prices[symbol]          # remove SPY
    df_prices["CASH"] = 1.0                # add CASH price

    ##################### df_trades #########################
    # 1) Build dataframe for trades with same structure as df_price
    ...
    row_num = 0
    while (row_num < df_orders.shape[0]):
        shares = df_orders.iloc[row_num].item()
        trade_date = df_orders.index[row_num]

        # Calculate total $ value of shares Bought or Sold in each order
        share_value = df_prices.loc[trade_date, symbol] * shares
        ...
        if share_value > 0:     # BUY
            df_trades.loc[trade_date, symbol] = shares
            df_trades.loc[trade_date, "CASH"] = df_trades.loc[trade_date, "CASH"] + share_value * (-1.0) - transaction_cost
        else:
        ...
        row_num += 1
    ##################### df_holdings #########################
    # 1) Build dataframe for holdings with same df_trades structure.
    ...
    df_holdings = pd.DataFrame(data=None, columns=df_trades.columns, index=df_trades.index).fillna(0)
    ...
    df_holdings.loc[df_trades.index[0]] = df_trades.iloc[0]
    ...
    row = 1 # Do not touch the first row (i.e., row = 0)
    while (row <= df_holdings.shape[0]-1):

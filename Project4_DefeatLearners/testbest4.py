...
def test_code():
    ...
    X, Y = best4LinReg(seed=903549376)
    X_LIN, Y_LIN = best4LinReg(seed=903549376)    #Repeat call with same/different seed to test similarity
    print((X == X_LIN).all())
    print((Y == Y_LIN).all())
    ...
    lrlearner = lrl.LinRegLearner(verbose = False)  		  	   		     			  		 			 	 	 		 		 	 		 		 	 		  	 	 			  	 
    dtlearner = dt.DTLearner(verbose = False, leaf_size = 1)  		  	   		     			  		 			 	 	 		 		 	 		 		 	 		  	 	 			  	 
    X, Y = best4DT(seed=903549376)
    X_DT, Y_DT = best4DT(seed=903549376)    #Repeat call with same/different seed to test similarity in generated dataset
    print((X == X_DT).all())
    print((Y == Y_DT).all())
